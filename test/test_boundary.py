...
def test_concave_polygon_sdf():
    ...
    assert float(sdf_notch[0]) > 0, (
        f"Notch point SDF should be positive, got {sdf_notch[0]}"
    )
    ...
    assert float(sdf_in[0]) < 0, (
        f"Interior point SDF should be negative, got {sdf_in[0]}"
    )
    ...
    assert float(sdf_out[0]) > 0, (
        f"Exterior point SDF should be positive, got {sdf_out[0]}"
    )
...
def test_exclusion_basic():
    ...
    assert float(pen_in) > 0, (
        f"Inside point should have positive exclusion penalty, got {pen_in}"
    )
    ...
    assert float(pen_out) == 0.0, (
        f"Far outside point should have zero penalty, got {pen_out}"
    )
...
def test_exclusion_with_buffer():
    ...
    assert float(pen) > 0, (
        f"Point within buffer should have positive penalty, got {pen}"
    )
    ...
    assert float(pen_far) == 0.0, (
        f"Point beyond buffer should have zero penalty, got {pen_far}"
    )
...
def test_differentiability():
    ...
    assert jnp.all(jnp.isfinite(grad_contain)), (
        f"containment grad not finite: {grad_contain}"
    )
    assert jnp.any(grad_contain != 0.0), (
        "containment grad should be nonzero for outside point"
    )
    ...
    assert jnp.all(jnp.isfinite(grad_exclude)), (
        f"exclusion grad not finite: {grad_exclude}"
    )
    assert jnp.any(grad_exclude != 0.0), (
        "exclusion grad should be nonzero for inside point"
    )
    ...
    assert jnp.all(jnp.isfinite(grad_concave)), (
        f"concave containment grad not finite: {grad_concave}"
    )
